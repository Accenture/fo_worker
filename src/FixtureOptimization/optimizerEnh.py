...
def optimizeEnh(methodology, jobType, jobName, Stores, Categories, increment, weights, 
    ...
    :param prepped_data: Output with intial data set inputs
    ...
    # print(prepped_data.columns)
    mergedPreOptCF = pd.merge(cfbsOutput, prepped_data[['Store', 'Category', 'VSG', 'Store Space', 'Penetration','Exit Flag','Sales Penetration','BOH $', 'Receipts  $','BOH Units', 'Receipts Units', 'Profit %','CC Count w/ BOH',]],on=['Store', 'Category'])
