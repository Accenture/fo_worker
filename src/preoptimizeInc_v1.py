...
def spreadCalc(sales,boh,receipt,master_columns):
     ...
     return calcPen(sales) + ((calcPen(sales) - calcPen(boh + receipt)) * float(spr_adj))
...
def metric_per_metric(metric1,metric2,spr_adj,master_columns):
    # storing input sales data in an array
		# finding penetration for each brand in given stores
		# calculate adjusted penetration
    metric1.columns = master_columns
    metric2.columns = master_columns
    return calcPen(metric1) + ((calcPen(metric1) - calcPen(metric2)) * float(spr_adj))
...
def metric_creation(data,bfc,moe,spr_adj,metric,increment):
    '''    
    ...
    if metric == 1: #spread
        ...
        adj_p = metric_per_metric(sales,bfc,spr_adj,getColumns(data))
        ...
        adj_p = metric_per_metric(profit,bfc,spr_adj,getColumns(data))
...
metric=6
increment=.25
adj_p = metric_creation(transaction_data, bfc,moe,spr_adj,metric,increment)
adj_p.head()
