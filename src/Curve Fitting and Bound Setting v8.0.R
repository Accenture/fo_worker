  ...
  ### These are all commented out because they are now defined in the python script
  ...
  # big_master_data<-read.csv("big_master_data_Input for Curve Fitting.csv",header=TRUE,sep=",")
  # bound_input<-read.csv("Bound_Input1.csv",header=TRUE,sep=",")
  
  #Curve Fitting and Bound Setting Function design
  curvefitting_boundsetting<-function(big_master_data,bound_input,strcount_filter,avgsales_flter,bucket_width,curve_split,sales_weight,profit_weight,units_weight,PCT_Space_Change_Limit,type){
  ...
  for(j in 1:nrow(big_master_data)){
    big_master_data$productivity_group_dummy[j]<-(sales_weight*big_master_data$Sales[j] + profit_weight*big_master_data$Profit[j] + units_weight*big_master_data$Units[j])/big_master_data$Space
  }
  ...
  big_master_data<-big_master_data[c("Store","Product","Store_Group","Space","Sales","Profit","Units","PCT_Space_Change_Limit","Space.Lower.Limit","Space.Upper.Limit","PCT_Space_Lower_Limit","PCT_Space_Upper_Limit","Exit","Future_Space","Entry_Space","Climate_Group","Productivity_Group","Climate","VSG")]
  ...
  for(b in (5:7)){
    ...
    master_data<-big_master_data[,c(1:4,b,8:17)]
    ...
    for (k in 1:nrow(data.frame(produclist))){
      ...
      coef[1,2]<-ifelse(coef[1,2]==0,0.25,coef[1,2])
    ...
    if (b==5){
      ...
      final_master<-merge(final_master,final,by=c("Store","Climate","VSG","Product","Store_Group","Space","BA_Space","Climate_Group","Productivity_Group"),all.x=TRUE)
    }
  }
  ...
  if(type=="Regular"){
  final_master$PCT_Change_Lower_Limit<-floor(final_master$Space*(1-final_master$PCT_Space_Change_Limit)/bucket_width)*bucket_width
  final_master$PCT_Change_Upper_Limit<-ceiling(final_master$Space*(1+final_master$PCT_Space_Change_Limit)/bucket_width)*bucket_width
  ...
  final_master$PCT_of_Space_Lower_Limit<-floor(final_master$PCT_Space_Lower_Limit*(final_master$Future_Space-final_master$Entry_Space)/bucket_width)*bucket_width
  final_master$PCT_of_Space_Upper_Limit<-ceiling(final_master$PCT_Space_Upper_Limit*(final_master$Future_Space-final_master$Entry_Space)/bucket_width)*bucket_width
  
  
  for(h in (1:nrow(final_master))){
  final_master$Lower_Limit[h]<-ifelse(final_master$Exit[h]=="TRUE",0,max(final_master$Space.Lower.Limit[h],final_master$PCT_Change_Lower_Limit[h],final_master$PCT_of_Space_Lower_Limit[h]))  
  ...
  final_master$Upper_Limit[h]<-ifelse(final_master$Exit[h]=="TRUE",0,min(upper_limit[which(upper_limit!=0)]))
  }
  ...
  #Function calling
  # curvefitting_boundsetting(big_master_data,bound_input,strcount_filter,avgsales_flter,bucket_width,curve_split,sales_weight,profit_weight,units_weight,PCT_Space_Change_Limit,type)
